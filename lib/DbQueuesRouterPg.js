...
const DbListenerPg = require ('./DbListenerPg')
...
class DbQueuesRouterPg {
	...
	set router (listener) {
		...
		if (!(listener instanceof DbListenerPg)) throw Error ('Not a DbListenerPg: ' + listener)

		const {pool} = listener = this.listener = listener

		for (const [_, db] of this.app.pools) if (pool.isSameDbAs (db)) {
