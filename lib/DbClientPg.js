...
class DbClientPg extends EventEmitter {
	...
	async getArray (sql, params = [], options = {}) {
		...
		let maxRows = options.maxRows || 1000
		...
		return new Promise ((ok, fail) => {
			...
			cursor.read (maxRows + 1, (cause, rows) => {
			
				cursor.close ()
				
				if (rows && rows.length > maxRows) cause = Error (maxRows + ' rows limit exceeded. Plesae fix the request or consider using getStream instead of getArray')
...
*/
	...
	async getObject (sql, params = [], options = {}) {
		...
		return new Promise ((ok, fail) => {
			...
			cursor.read (1, (cause, rows) => {
			
				cursor.close ()
				
				if (!cause && rows && rows.length === 0 && notFound instanceof Error) cause = notFound
				...
				if (cause) {
					...
					ok (rows.length === 0 ? ('notFound' in options ? notFound : {}) : rows [0])
	...
	async getScalar (sql, params = [], options = {}) {
		...
		const r = await this.getObject (sql, params, {
			notFound: undefined,
			...options,
			rowMode: 'array',
		})
