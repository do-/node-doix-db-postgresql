...
const TYPE_MAP = new Map (Object.entries (require ('pg-types').builtins).map (([name, id]) => ([id, name.toLowerCase ()])))
			...
			cursor.read (isPartial ? maxRows : maxRows + 1, (cause, rows) => {
				...
				if (cause) {
					...
					Object.defineProperty (rows, Symbol.for ('columns'), {
						configurable: false, 
						enumerable: false, 
						get: () => cursor._result.fields.map (i => {i.type = TYPE_MAP.get (i.dataTypeID); return i})
					})
