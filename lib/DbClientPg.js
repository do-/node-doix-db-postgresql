	...
	async exec (call) {
		...
		switch (maxRows) {
			...
			case Infinity : 
				...
				const stream = await raw.query (new PgQueryStream (sql, params, o))

				if (this.lang.isCopyStatement (sql)) stream.handleCopyData = function ({chunk}) {this.push (chunk)}

				return call.rows = stream
