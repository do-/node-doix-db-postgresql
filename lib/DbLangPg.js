...
class DbLangPg extends DbLang {
	...
	* genDDL () {
		...
		const {model} = this.migrationPlan

		for (const view of model.allInstancesOf (DbView)) yield [this.genReCreateViewAsMock (view)]

		for (const view of model.allInstancesOf (DbView)) yield [this.genReCreateView (view)]
