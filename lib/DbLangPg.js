const {DbLang, DbRelation, DbTable, DbView, DbProcedure, DbFunction} = require ('doix-db')
...
class DbLangPg extends DbLang {
	...
	* genCreateIndexes () {
		...
		for (const o of toBe.values ()) if (o instanceof DbRelation && 'keys' in o) {

			const {keys} = o

			for (const k in keys) yield this.genCreateIndex (keys [k])
