...
class DbLangPg extends DbLang {
	...
	constructor () {
		...
		this.TYPE_MAP = new Map (Object.entries (require ('pg-types').builtins)

			.map (([name, id]) => ([id, name.toLowerCase ()])))
	...
	* genDropViews (model) {
		...
		const qNames = []; for (const o of model.map.values ())
		
			if (o instanceof DbView)

				qNames.push (o.qName)
				
		if (qNames.length === 0) return

		yield [`DROP VIEW IF EXISTS ${qNames} CASCADE`]

	}
	
	* genCreateViews (model) {

		for (const o of model.map.values ())
		
			if (o instanceof DbView)

				yield [`CREATE VIEW ${o.qName} AS ${o.sql}`]
